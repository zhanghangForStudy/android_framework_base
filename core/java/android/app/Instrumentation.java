...
//      }
     ...
     * objects and dispatches（分派） this call to the system activity manager; you can
     * override this to watch for the application to start an activity, and
     * modify what happens when it does.
     * 执行一个被应用构造的一个startActivity调用。默认的执行只关注更新活跃对象和分派此调用给AMS；
     * 可以重写此方法以找寻启动此activity的应用。
     ...
     * 此方法返回一个ActivityResutl对象，当需要避免启动新activity但是仍然需要返回应用期望的结果，而进行此调用拦截时，
     * 可以使用此对象。
     * 为了实现这种情景，重写此方法，捕获启动新activity的调用，以便能够返回一个新的，
     * 包含了你希望应用看见的结果的ActivityResult对象
     * <p>This method throws {@link android.content.ActivityNotFoundException}
     * if there was no Activity found to run the given Intent.
     *
     * @param who The Context from which the activity is being started.
     * @param contextThread The main thread of the Context from which the activity
     *                      is being started.旧Activity所在进程的引用，为AMS所用
     * @param token Internal token identifying to the system who is starting
     *              the activity; may be null.系统可识别的旧activity引用，及AMS中的ActivityRecord引用
     * @param target Which activity is performing the start (and thus receiving
     *               any result); may be null if this call is not being made
     *               from an activity.旧activiy
     * @param intent The actual Intent to start.标示如何启动的Intent
     * @param requestCode Identifier for this request's result; less than zero
     *                    if the caller is not expecting a result.如果小于0，表示旧Activity不期望结果
     * @param options Addition options.
     *
     * @return To force the return of a particular（特别的，详细的） result, return an
